...
#define LSM6DS3TR_ADDRESS                    0x6Au  //!< 7 bit Device address

#define SLAVE_ADDRESS                 (LSM6DS3TR_ADDRESS | SA0_PIN_STATE)<<1  //!< Slave address
...
#define FUNC_SRC1_REG_ADDRESS				0x53u	// Significant motion, tilt, step detector, hard/soft-iron and sensor hub interrupt source register
...
uint8_t I2C_ReadFromAddress(uint8_t addr, uint8_t reg, uint8_t* data, uint8_t num) {
	uint8_t i = 0;
	uint8_t ret = 0;
...
void I2C_WriteToAddress(uint8_t addr, uint8_t reg, uint8_t* data, uint8_t num) {
...
void LSM6DS3TR_ReadAcceleration(int8_t* data)
{
	I2C_ReadFromAddress(SLAVE_ADDRESS, OUTX_L_XL_REG_ADDRESS, (uint8_t*)data, 6u);
}

void LSM6DS3TR_ReadAccelerationXY(int8_t* data)
{
	I2C_ReadFromAddress(SLAVE_ADDRESS, OUTX_L_XL_REG_ADDRESS, (uint8_t*)data, 4u);
}
...
void LSM6DS3TR_InitGyroscope(int16_t offset)
{
	...
	I2C_WriteToAddress(SLAVE_ADDRESS, CTRL7_G_REG_ADDRESS, data, 1u);
...
void LSM6DS3TR_ReadAngularVelocity(int8_t* data)
{
	I2C_ReadFromAddress(SLAVE_ADDRESS, OUTX_L_G_REG_ADDRESS, (uint8_t *)data, 6u);
}

extern void LSM6DS3TR_ReadAngularVelocityZ(int8_t* data) {
	I2C_ReadFromAddress(SLAVE_ADDRESS, OUTZ_L_G_REG_ADDRESS, (uint8_t *)data, 2u);
}
...
uint8_t LSM6DS3TR_CheckManufacturerId(void)
{
  uint8_t id[1] = {0x00u};
...
void LSM6DS3TR_InitLIS2ML(void) {
	...
	I2C_WriteToAddress(SLAVE_ADDRESS, CTRL1_XL_REG_ADDRESS, data, 1);
	
	data[0] = 0x80; // Enable access to embedded functions registers (bank A)
	I2C_WriteToAddress(SLAVE_ADDRESS, FUNC_CFG_ACCESS_REG_ADDRESS, data, 1);
	
	data[0] = LIS2MDL_ADDR+I2C_WRITE; // LIS2MDL slave address, Enable write operation (rw_0=0)
	I2C_WriteToAddress(SLAVE_ADDRESS, SLV0_ADD_REG_ADDRESS, data, 1);

	data[0] = 0x60; // 60h is the LIS2MDL register to be written
	I2C_WriteToAddress(SLAVE_ADDRESS, SLV0_SUBADD_REG_ADDRESS, data, 1);
	...
	I2C_WriteToAddress(SLAVE_ADDRESS, DATAWRITE_SRC_MODE_SUB_SLV0_REG_ADDRESS, data, 1);
	
	data[0] = 0x10; // Set Aux_sens_on bits different from 00b
	I2C_WriteToAddress(SLAVE_ADDRESS, SLAVE0_CONFIG_REG_ADDRESS, data, 1);

	data[0] = 0x20; // Enable write_once bit
	I2C_WriteToAddress(SLAVE_ADDRESS, SLAVE1_CONFIG_REG_ADDRESS, data, 1);

	data[0] = 0x00; // Disable access to embedded functions registers (bank A)
	I2C_WriteToAddress(SLAVE_ADDRESS, FUNC_CFG_ACCESS_REG_ADDRESS, data, 1);
	
	data[0] = 0x04; // Enable embedded functions
	I2C_WriteToAddress(SLAVE_ADDRESS, CTRL10_C_REG_ADDRESS, data, 1);	
	...
	while(trials < 3) {
		...
		I2C_WriteToAddress(SLAVE_ADDRESS, MASTER_CONFIG_REG_ADDRESS, data, 1);

		data[0] = 0x80; // Turn on the accelerometer (for trigger signal)
		I2C_WriteToAddress(SLAVE_ADDRESS, CTRL1_XL_REG_ADDRESS, data, 1);

		// Wait for the sensor hub communication to be concluded
		I2C_ReadFromAddress(SLAVE_ADDRESS, FUNC_SRC1_REG_ADDRESS, data, 1u);
		if((data[0]&0x01)==0x01) {
			break;
		}
	...
	data[0] = 0x00; // Disable embedded functions
	I2C_WriteToAddress(SLAVE_ADDRESS, CTRL10_C_REG_ADDRESS, data, 1);	
	...
	data[0] = 0x3C; // Enable embedded functions + enable all gyro axes.
					// The datasheet doesn't describe the bit related to the gyro axes enabling, but from
					// field tests they are bi5, bi4 and bit3.
	I2C_WriteToAddress(SLAVE_ADDRESS, CTRL10_C_REG_ADDRESS, data, 1);
...
/*
	...
	// X=34, Y=-149, Z=283
	data[0] = 0x00;
	I2C_WriteToAddress(SLAVE_ADDRESS,  MAG_OFFX_H_REG_ADDRESS, data, 1); // X offset value initialization
	data[0] = 0x22;
	I2C_WriteToAddress(SLAVE_ADDRESS,  MAG_OFFX_L_REG_ADDRESS, data, 1); // X offset value initialization
	data[0] = 0xFF;
	I2C_WriteToAddress(SLAVE_ADDRESS,  MAG_OFFY_H_REG_ADDRESS, data, 1); // Y offset value initialization
	data[0] = 0x6B;
	I2C_WriteToAddress(SLAVE_ADDRESS,  MAG_OFFY_L_REG_ADDRESS, data, 1); // Y offset value initialization
	data[0] = 0x01;
	I2C_WriteToAddress(SLAVE_ADDRESS,  MAG_OFFZ_H_REG_ADDRESS, data, 1); // Z offset value initialization
	data[0] = 0x1B;
	I2C_WriteToAddress(SLAVE_ADDRESS,  MAG_OFFZ_L_REG_ADDRESS, data, 1); // Z offset value initialization	
	...
	//	0	0	8
	data[0] = 0x08;
	I2C_WriteToAddress(SLAVE_ADDRESS,  MAG_SI_XX_REG_ADDRESS, data, 1); // XX soft-iron element
	data[0] = 0x00;
	I2C_WriteToAddress(SLAVE_ADDRESS,  MAG_SI_XY_REG_ADDRESS, data, 1); // XY soft-iron element
	data[0] = 0x00;
	I2C_WriteToAddress(SLAVE_ADDRESS,  MAG_SI_XZ_REG_ADDRESS, data, 1); // XZ soft-iron element
	data[0] = 0x00;
	I2C_WriteToAddress(SLAVE_ADDRESS,  MAG_SI_YX_REG_ADDRESS, data, 1); // YX soft-iron element
	data[0] = 0x08;
	I2C_WriteToAddress(SLAVE_ADDRESS,  MAG_SI_YY_REG_ADDRESS, data, 1); // YY soft-iron element
	data[0] = 0x00;
	I2C_WriteToAddress(SLAVE_ADDRESS,  MAG_SI_YZ_REG_ADDRESS, data, 1); // YZ soft-iron element
	data[0] = 0x00;
	I2C_WriteToAddress(SLAVE_ADDRESS,  MAG_SI_ZX_REG_ADDRESS, data, 1); // ZX soft-iron element
	data[0] = 0x00;
	I2C_WriteToAddress(SLAVE_ADDRESS,  MAG_SI_ZY_REG_ADDRESS, data, 1); // ZY soft-iron element
	data[0] = 0x08;
	I2C_WriteToAddress(SLAVE_ADDRESS,  MAG_SI_ZZ_REG_ADDRESS, data, 1); // ZZ soft-iron element				
*/
...
*/
	...
	//data[0] = 0xFF; // Enable all acc axes + enable soft-iron correction (doesn't correspond to datasheet...)
	//I2C_WriteToAddress(SLAVE_ADDRESS, CTRL9_XL_REG_ADDRESS, data, 1);

}

void LSM6DS3TR_ReadLIS2ML(int8_t* data) {
	I2C_ReadFromAddress(SLAVE_ADDRESS, SENSORHUB1_REG_ADDRESS, (uint8_t*)data, 6u);
}

uint8_t LSM6DS3TR_ReadAll(int8_t* data) {
	return I2C_ReadFromAddress(SLAVE_ADDRESS, OUTX_L_G_REG_ADDRESS, (uint8_t *)data, 18);
}
